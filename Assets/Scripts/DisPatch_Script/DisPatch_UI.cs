...
public class DisPatch_UI : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        _portalName = transform.Find("PortalInfo/PortalName").GetComponent<Text>();

        ConstructionManager.Instance.OnConstructionClicked.AddListener((construction) =>
        {
            if (construction is Portal)
            {
                Setting_UI(construction as Portal);
    ...
    public void Setting_UI(Portal portal)
    {
        ...
        //포탈 이름 UI에 표시
        _portalName.text = targetPortal.portalName;
        //포탈 능력치 UI에 표시
